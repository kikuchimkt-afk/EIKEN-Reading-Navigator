// Script to append Bias Algorithms to readingData.js
const fs = require('fs');

// Read the base JSON
const baseData = JSON.parse(fs.readFileSync('bias_algorithms_complete.json', 'utf8'));

// Add questions and summary
baseData.questions = [
    {
        id: 1,
...
baseData.summaryForInstructors = {
...
// Save complete data
fs.writeFileSync('bias_algorithms_final.json', JSON.stringify(baseData, null, 4), 'utf8');
console.log('Created bias_algorithms_final.json with questions and summary');

// Now read readingData.js and append
const readingDataPath = './src/data/readingData.js';
let readingDataContent = fs.readFileSync(readingDataPath, 'utf8');

// Find the position to insert (before the last ] and any trailing content)
const lastBracketIndex = readingDataContent.lastIndexOf('];');

if (lastBracketIndex === -1) {
    console.error('Could not find end of readingData array');
    process.exit(1);
}

// Create the new entry as a string
const newEntry = `,
    // 準1級 オリジナル問題13: The Bias in Algorithms
    ${JSON.stringify(baseData, null, 4).replace(/\n/g, '\n    ')}`;

// Insert before the ];
const newContent = readingDataContent.slice(0, lastBracketIndex) + newEntry + '\n' + readingDataContent.slice(lastBracketIndex);

fs.writeFileSync(readingDataPath, newContent, 'utf8');
console.log('Successfully appended to readingData.js');
