...
def generate_structure_html(chunks):
    """
    chunks: list of tuples (text, tag)
    e.g. [("In 1946", "M"), (",", ""), ("the press", "S"), ...]
    """
    html = ""
    for text, tag in chunks:
        if tag:
            html += f"<span class='chunk {tag}-group'><span class='text'>{text}</span><span class='tag'>{tag}</span></span>"
        else:
            html += text
    return html

def generate_translation_html(chunks):
    """
    chunks: list of tuples (text, tag)
    e.g. [("1946年に", "t-M"), ("、", ""), ("報道陣は", "t-S"), ...]
    """
    html = ""
    for text, tag in chunks:
        if tag:
...
js_content = f"""
// Generated Data for Plant Communication
const plantCommunicationData = {encoded_data};

// Append to readingData.js logic would go here
// This file is just to verify the structure and content generation.
console.log(JSON.stringify(plantCommunicationData, null, 2));
"""

with open('plant_data_gen.js', 'w', encoding='utf-8') as f:
    f.write(js_content)
