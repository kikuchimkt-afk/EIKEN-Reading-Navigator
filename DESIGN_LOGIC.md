...
本ドキュメントは、英検長文読解問題アプリ（EIKEN Reading Navigator）における、ユーザーインタフェースおよび学習体験の演出ロジックを定義するものです。以後の開発・データ追加は本指針に従って行います。
...
*   **Step 2 (解答選択)**:
    ...
    *   選択肢には `choiceAdvice`（回答のコツ）を箇条書きで表示し、選択肢の共通点や相違点に気付かせる。
*   **Step 4 (正解ロジック)**:
...
*   **言い換え (Paraphrase)**:
    ...
    *   **動的ハイライト**: ユーザーが操作パネルでキーワードをクリックした際、本文中に含まれるその単語（大文字小文字区別なし）は解答に関係するすべてパラグラフ内の該当箇所がハイライトされる。複数選択時はすべてのキーワードがハイライト対象となる。
